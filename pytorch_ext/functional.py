...
def dropout(input, mask=None, p=0.5, shared_axes=(), rescale=True, fill_value=0.0, return_mask=False):
    """
    Dropout function for any dimension input.
    :param input: tensor to be dropout
    :param mask: if given, input of `p`/`shared_axes`/`rescale` will be ignored
    ...
    :param return_mask: whether return the dropout mask used inside
    """
    if mask is None:
